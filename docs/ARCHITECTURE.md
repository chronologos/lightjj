...
**3-pane merge editor** (`MergePanel.svelte`) — The primary interactive conflict path. Clicking Merge on a 2-side conflicted file fetches raw marker content via `api.fileShow()`, `conflict-extract.ts` reconstructs `{base, ours, theirs}` from jj-native markers (`Diff`/`Snapshot` styles; `Git` style + 3+ sides fall through to raw FileEditor). Three CM6 `EditorView`s (ours | result | theirs): flanks read-only, center seeded with `theirs` and editable. `merge-diff.ts` line-LCS computes per-block change ranges; arrow buttons copy flank content into the center at the tracked block position. A `blockTracker` StateField maps block positions through user edits (CM6 `mapPos` with assoc 1/-1, degenerate-range clamp for select-all-and-type). `takeBlock` handles `\n` separator surgery at doc boundaries (`srcEmpty` — not `!insert`, blank lines are valid content). Undo inverse via `invertedEffects` so Cmd+Z restores text AND source tag. Cmd+S → `POST /api/file-write` (same path as FileEditor, not `/api/resolve`). The button-click `:ours`/`:theirs` path remains for quick resolution.
