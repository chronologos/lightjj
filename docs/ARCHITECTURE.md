...
The server includes an operation ID cache that tracks jj's current operation, encapsulated behind `getOpId()`/`setOpId()`/`casOpId()` (server.go) — `casOpId` is the compare-and-swap the SSH poll loop uses so a poll result captured before a concurrent mutation's refresh can't regress the cache. Every JSON response includes an `X-JJ-Op-Id` header. Mutation endpoints refresh the cache synchronously via `runMutation()`, which centralizes the post-mutation pattern (run command → refresh op ID → return output).
...
**Bookmark split** — The template concatenates `local_bookmarks` + `remote_bookmarks` (NOT the merged `bookmarks` keyword — that collapses tracked-and-synced remotes into the local form, so `main@upstream` vanishes when local `main` is at the same commit). Remote entries use `\x1E` as name/remote sub-separator (`b.name() ++ "\x1E" ++ b.remote()`). The parser splits at `\x1E`, filters the `@git` colocation synthetic remote, strips RefSymbol quotes, and emits `GraphRow.Bookmarks []string` (local only) + `GraphRow.RemoteBookmarks []RemoteRef{Name, Remote}`. `\x1E` never crosses the wire — it's a template serialization detail. This replaces a single mixed `name@remote` array where naive `@` splitting broke on git-created branch names containing `@`.
...
**Merge view** (`activeView='merge'`) — Top-level view alongside log/branches (a *view*, not a mode — "mode" is reserved for the inline rebase/squash/split states). `switchToMergeView()` fetches `GET /api/conflicts` (jj log over `conflicts()` revset emitting per-commit conflicted files + side counts). `ConflictQueue.svelte` renders the left rail: flat file list grouped by commit, ○/● resolved dots, N-way badges, j/k navigation. Selecting a file loads it into `MergePanel` via `{#key commitId:path}` remount. RevisionGraph is hidden entirely in merge view — the queue + 3-pane editor need full width.

**3-pane merge editor** (`MergePanel.svelte`) — Per-file conflict editor. Raw marker content from `api.fileShow(rev, path, true)` (the `?snapshot=1` param forces `ui.conflict-marker-style=snapshot` so base is byte-exact in `-------` sections, no `%%%%%%%` diff reconstruction; opt-in only for the merge view — hunk-review must match user's style) goes through `conflict-extract.ts:reconstructSides()` which parses jj-native markers (`Diff`/`Snapshot` styles; `Git` style + 3+ sides fall through to raw FileEditor) into `{base, ours, theirs, blocks}`. The `blocks` array — one `ChangeBlock` per `<<<<<<<`/`>>>>>>>` region — is recorded at parse time; no LCS needed since the marker state machine already knows the boundaries. Three CM6 `EditorView`s (ours | result | theirs): flanks read-only, center seeded with `theirs` and editable. Arrow buttons copy flank content into the center at the tracked block position (`planTake` / `planTakeBoth` in `merge-surgery.ts`). A `blockTracker` StateField maps block positions through user edits (CM6 `mapPos` with assoc 1/-1, degenerate-range clamp for select-all-and-type). `takeBlock` handles `\n` separator surgery at doc boundaries (`srcEmpty` — not `!insert`, blank lines are valid content). Undo inverse via `invertedEffects` so Cmd+Z restores text AND source tag. `[`/`]` block nav + "N of M" pill + minimap. Cmd+S → `merge-controller.save()`: `@` via `POST /api/file-write` (SSH-compatible), non-@ via `POST /api/merge-resolve` (`jj resolve --tool cp`, local-only — 501 in SSH).

**File history** (`FileHistoryPanel.svelte`) — Two-cursor compare overlay. Right-click a diff line → "View history" → `api.fileHistory(path)` populates a `FileHistoryRail` (extracted two-tier loader: mutable-scoped first for speed, full-history on demand via `jj debug index-changed-paths`). j/k moves cursor B, Space pins cursor A, `api.diffRange(A, B, [path])` renders the right-side diff via `createLoader`. `FileComparePicker` provides the same rail inline in DiffPanel for ad-hoc comparison without the overlay.

### Inline file editing

`FileEditor.svelte` wraps CodeMirror 6 in the split-view right column. Clicking Edit in unified view auto-switches to split (via `splitView = $bindable` write-back). Indent detection scans the first 200 indented lines to configure `indentUnit` (tabs vs N-spaces); `tabSize=4` matches `.diff-line { tab-size: 4 }` so columns align. Hunk folding collapses unchanged regions with 3 lines of context. `POST /api/file-write` writes directly to the filesystem — no jj command. The snapshot loop (`watcher.go`, 5s interval) picks up the write via `jj debug snapshot`, which advances op_heads → fsnotify fires → SSE pushes the new op-id → frontend reloads. Editing is capability-gated on `diffTarget.kind === 'single'` — in multi-check mode the cursor position isn't what's being diffed, so `api.edit(cursor's changeId)` would target the wrong revision.
...
| Layer | Count (v1.12) | Coverage |
|-------|-------|----------|
| Go unit | ~180 | Command builders, parsers (FilesTemplate, BookmarkList, GraphLog, WorkspaceList), models (`Commit.GetChangeId`, `SelectedRevisions`), runner arg-escaping |
| Go handlers | ~260 | Every endpoint's happy path + validation (400) + runner error (500) via `runnerErrorTest` helper |
| Go integration | ~35 | Build-tagged. CRUD journey, divergence resolution, diff-range file filtering, bookmark lifecycle |
| Frontend pure logic | ~950 | **Extract-and-pin** modules (below) + api.ts cache/op-id, diff-parser, word-diff LCS, split-view alignment, loader races, mode factories, keyboard-gate ordering |
...
6. **`--tool :git` for diffs** — Users may have external diff tools configured (e.g., difftastic with `--color=always`). The web API forces jj's git-format diff output to get clean, parseable output.

7. **`--ignore-working-copy` on reads** — Without it, every `jj log`/`file show` re-stats every tracked file (~485ms in a large repo, measured). `watcher.go` already snapshots every `--snapshot-interval` — `snapshotLoop` via `jj util snapshot` locally, `sshPollLoop` via `PollOpId` (implicit snapshot) in `--remote` mode — so read-path snapshots are redundant work that also contends on the WC lock. Concurrent snapshotting commands serialize (measured 85% CPU for 5 parallel logs); `--ignore-working-copy` commands parallelize freely (221% CPU). Worst case: an external file edit is visible ≤5s late before the SSE correction arrives — the same contract every other read already has. Both loops are standard jj commands — `snapshot.auto-update-stale` (jj's own default) detects stale WCs in multi-workspace repos and checks out the new tree before snapshotting.

8. **Immutable commit detection via graph glyphs** — The graph parser checks for `◆` vs `○` vs `@` when parsing node rows. `◆` sets `Immutable: true` on the `Commit` struct. The frontend uses this to dim immutable commits and color gutter symbols (`○` blue, `@` green) without needing a separate API call.

9. **Per-remote visibility** — BookmarksPanel (key `2`) groups bookmarks by remote with per-group and per-bookmark eye toggles. Toggling a remote builds an `ancestors(remote_bookmarks(remote="X"), 2)` revset; per-bookmark enumeration emits `"name"@"remote"` (the `@` MUST stay unquoted — it's the revset AT operator, not part of a string literal). The resulting revset auto-populates the graph filter only when the user hasn't typed a custom query (tracked via `prevVisibilityRevset` comparison — not `viewMode`, which would lazy-recompute with new visibility + old filter and stall on `'custom'`). Replaces the old Log/Tracked toggle.

10. **Op-ID staleness detection** — Every response carries `X-JJ-Op-Id`. The frontend detects operation changes and auto-refreshes. Mutation endpoints refresh the cached op-id asynchronously to avoid adding latency.

11. **Divergent commit identity** — Divergent commits share the same `change_id`, so the frontend uses `effectiveId()` (falls back to `commit_id`) for identity operations. The `change_id()` revset function (not `all:` which doesn't exist in jj 0.38) resolves all divergent versions. Divergence offsets (`/0`, `/1`) preserve jj's emission order (`GlobalCommitPosition` — index-insertion order, NOT commit_id sort). The frontend does not re-sort; doing so would mislabel which copy `jj restore -r abc/0` targets. The DivergencePanel is self-fetching — it receives only a `changeId` and manages its own data loading with separate generation counters for version fetching and diff fetching.
