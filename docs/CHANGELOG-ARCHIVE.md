# lightjj — Design Decision Archive

Dense narrative of what was built and why. The `[x]` prose encodes hard-won
design rationale (why factories were rejected, which races were real, what
got tried and reverted). Kept out of BACKLOG.md so that file stays scannable.

**Do not treat the remaining `[ ]` items here as a backlog** — they're the
historical context the `[x]` items were discussed alongside. The LIVE
actionable backlog is [BACKLOG.md](../BACKLOG.md).
...
- [ ] **Audit localStorage usage for port-randomization breakage** (Small) — localStorage is scoped to origin (`localhost:PORT`). lightjj randomizes ports on restart → all localStorage state lost each launch. Audit `recent-actions.svelte.ts`, `config.svelte.ts` write-through cache, and any other localStorage usage. Either migrate to server-side config or accept the ephemeral semantics and document.
...
- [x] **Three-way merge editor** — `MergePanel.svelte` (811L) + `conflict-extract.ts` + `merge-diff.ts`. No `@codemirror/merge` dep — 3 raw `EditorView`s (ours | result | theirs), center seeded with theirs. `blockTracker` StateField tracks per-block position through edits (CM6 change-mapping + `mapPos` assoc, degenerate-range clamp for select-all-and-type). `takeBlock` arrow-click surgery (idempotent same-side, leading/trailing `\n` separator math, `srcEmpty` not `!insert` — blank lines are valid content). `applyBlock`/`editInside`/`restoreBlock` StateEffects + `invertedEffects` undo inverse (Cmd+Z restores text AND source tag). `reconstructSides()` parses jj-native markers (`Diff`/`Snapshot` styles; `Git` style + 3+ sides fall through to raw FileEditor). `conflict-extract.test.ts` 20 cases, `merge-diff.test.ts` 18 cases. MergePanel itself untested (CM6 integration — the inputs are well-tested).
