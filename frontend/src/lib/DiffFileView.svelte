...
<div class="diff-file" data-file-path={filePath} bind:this={fileEl} class:hunk-reviewing={!!hunkReview} class:file-reviewed={reviewed} onpointermove={symbolHover ? handleSymbolMove : undefined} onpointerleave={symbolHover ? () => symbolHover.leave() : undefined}>
  <div
    ...
    onclick={(e: MouseEvent) => {
      if (e.altKey && onannotationclick) { e.stopPropagation(); onannotationclick(FILE_LEVEL, '', e); return }
      ontoggle(filePath)
    }}
  ...
  >
    ...
    {#if onreviewedtoggle && !hunkReview}
      <button
        ...
        class:is-reviewed={reviewed}
        onclick={(e: MouseEvent) => { e.stopPropagation(); onreviewedtoggle(filePath, !reviewed) }}
        title={reviewed ? 'Reviewed — click to unmark' : 'Mark file as reviewed'}
        aria-label="Mark file as reviewed"
    ...
    {#if onopendoc && isMarkdown && !editing && fileStats?.type !== 'D'}
      <button class="btn btn-sm" class:has-doc-comments={docCommentCount > 0} onclick={(e: MouseEvent) => { e.stopPropagation(); onopendoc(filePath) }} title="Open as document (rich editor + comments){docCommentCount ? ` — ${docCommentCount} open` : ''}">
        Doc{#if docCommentCount > 0}&nbsp;<span class="doc-count-badge">{docCommentCount}</span>{/if}
      </button>
    {/if}
...
<style>
  ...
  .reviewed-check {
    ...
    font-weight: 700;
    line-height: 1;
    transition: border-color var(--anim-duration) var(--anim-ease), background var(--anim-duration) var(--anim-ease);
  }
  .reviewed-check:hover { border-color: var(--overlay1); }
  .reviewed-check.is-reviewed {
    border-color: var(--green);
    background: color-mix(in srgb, var(--green) 20%, transparent);
  }

  .file-reviewed .diff-file-header {
