...
- **Model "data is stale" as derived state (rendered-op-id vs current-op-id), not as a droppable event** — an `$effect` over the comparison re-fires when a suppressing gate clears; refresh-inside-the-callback loses the fact forever.
