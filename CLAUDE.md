...
frontend/                  — Svelte 5 SPA (Vite + TypeScript + pnpm)
  ...
  src/lib/
    ...
    MergePanel.svelte      — 3-pane conflict editor (ours | result | theirs). 3 raw CM6 EditorViews — no @codemirror/merge dep. Position surgery (takeBlock separator math, blockTracker mapPos + inversion) EXTRACTED to merge-surgery.ts. applyBlock/editInside/restoreBlock StateEffects + invertedEffects undo inverse (Cmd+Z restores text AND source tag). Center seeded with theirs.
    merge-surgery.ts       — PURE — planTake() = takeBlock's separator-math (leading/trailing \n, empty-line insert-as-is, srcEmpty deletion + \n consumption). remapBlock() = mapPos with whole-block-replace inversion. initialTrackPos() = 1-indexed line → 0-indexed char. Testable without EditorView; merge-surgery.test.ts has round-trip (ours→theirs→identity) invariants + full conflict→diff→take pipeline tests.
    ...
    conflict-extract.ts    — reconstructSides(): parses jj-native conflict markers (Diff/Snapshot styles, {7,} length — jj escalates) into {base, ours, theirs}. Git-style markers + 3+ sides → null (fall through to raw FileEditor). State machine over < % + - > markers; % section uses ` `/`-`/`+` single-char prefixes per line.
    merge-diff.ts          — diffBlocks(): line-level LCS for 3-pane highlighting. ChangeBlock = contiguous differing range. Bails at 500k-cell matrix (same spirit as word-diff's cap).
