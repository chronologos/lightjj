...
- [ ] **Context-menu "Split..." while already in squash** (Low, edge) — App.svelte:602 context menu item isn't gated on `!inlineMode` (keyboard `S` and palette entry ARE gated). Clicking it during squash → `cancelInlineModes()` (calls `blurActiveInput()`) + `split.enter()` in same tick → FileSelectionPanel stays mounted (`{#if fileSelectionMode}` never goes false) but loses focus → j/k/Space dead. Fix: add `when: () => !inlineMode` to context-menu items, OR `{#key fileSelectionMode}` around the panel in DiffPanel. Deferred — requires an unusual interaction path.
...
- [ ] **Extract `WorkspaceSpawner`** (Small) — `spawnedWorkspaces`/`children`/`childrenMu` + `spawnLocked`/`Shutdown` (~90 LOC, server.go:318-395) are process-management squatting on the HTTP server struct. Self-contained, only reads `RepoDir` via `readWorkspaceStore`. Moot if "Replace spawnWorkspaceInstance with in-process tab" (below) lands first.
