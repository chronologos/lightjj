...
- [ ] **RevisionGraph virtualization** (Medium, scoped 2026-03-06) — 500 commits × ~3 lines each (node/desc/connectors) = ~1500 `.graph-row` DOM elements, each with a `<GraphSvg>` child. `scrollIntoView` on j/k recalculates styles on all of them. **Favorable preconditions:** rows are fixed `height: 18px` (enforced for graph pipe continuity per CLAUDE.md), so `@tanstack/svelte-virtual` with `estimateSize: () => 18` is the simplest virtualization case — no dynamic measurement, no ResizeObserver. **Scope:** (1) `createVirtualizer` over `flatLines` with `.panel-content` as scroll element; (2) replace `{#each flatLines}` with `{#each virtualizer.getVirtualItems()}`, apply `transform: translateY(item.start)` and container `height: virtualizer.getTotalSize()`; (3) replace `scrollIntoView` at RevisionGraph.svelte:221 with `virtualizer.scrollToIndex` — needs a `selectedIndex → firstLineIdx` derived map since selection is by `entryIndex` but virtualizer scrolls by `flatLines` index; (4) `hoveredIndex` via `mousemove` + `.closest('[data-entry]')` still works unchanged (virtual rows keep the attr). **Tests:** existing RevisionGraph.test.ts (39 tests) exercise row rendering/selection/hover — rerun after refactor; add one test asserting `<~50` rendered rows for a 200-commit input. **Effort:** ~half day. Biggest remaining architectural perf item.
...
- [ ] List virtualization for large repos — see scoped item in Remaining-Performance above
