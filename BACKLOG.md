...
- [ ] **Restore on renames — source-path plumbing** (Small) — Discard is currently hidden on `type === 'R'` because `jj restore -c X file:"<dest>"` only matches the new path (empirically: rename becomes a delete). Fix: parse `rename from <src>` from git diff header (`diff-parser.ts` — the line is present in `jj diff --tool :git` output), add `sourcePath?: string` to `DiffFile`, pass `[sourcePath, destPath]` to `api.restore()` when `type === 'R'`. Copy (`C`) is fine as-is — source exists at dest commit so deleting copy-dest IS the correct discard.
